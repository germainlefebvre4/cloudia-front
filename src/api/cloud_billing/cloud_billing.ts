...
export const fetchCloudBillingForChart = (start_year: number, start_month: number, end_year: number, end_month: number) => {
  return $axios.get<unknown, ICloudBillingForChart>(`${import.meta.env.VITE_APP_BASE_URL}/billing/aws/projects/chart?start_year=${start_year}&start_month=${start_month}&end_year=${end_year}&end_month=${end_month}`);
}
